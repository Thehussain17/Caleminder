...
class Orchestrator:
    def __init__(self):
        ...
        put_task_declaration = types.FunctionDeclaration(
            ...
            properties={
                ...
                "category": types.Schema(
                    type=types.Type.STRING,
                    description="The category or list for the task (e.g., 'Work', 'Personal'). Defaults to 'General' if not specified. A new list will be created if it doesn't exist."
                ),
                "due": types.Schema(
                    type=types.Type.STRING,
                    description="Optional due date and time for the task in RFC 3339 format (e.g., '2025-08-30T15:00:00.000Z')."
                ),
            },
            required=["title"]
            )
        ...
        self.tools = types.Tool(function_declarations=[put_task_declaration,get_upcoming_tasks_declaration,get_events_declaration,get_events_by_date_declaration,create_event_declaration, get_now_declaration, list_events_declaration, remove_event_declaration, schedule_timetable_declaration])
    ...
    def _generate_with_retry(self, **kwargs):
        """
        --- NEW: A robust method to call the Gemini API with exponential backoff. ---
        This makes the application resilient to temporary server overloads.
        """
        max_retries = 3
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Check if the error is a 503 Unavailable (overloaded model)
                if "503" in str(e) and "UNAVAILABLE" in str(e):
                    ...
                    # If it's a different error, don't retry, just fail fast
                    raise e
        return None
    ...
    def handle_message(self, message):
        ...
        try:
            ...
            current_request_conversation = history + contents
            ...
            while True:
                ...
                for function_call in function_calls:
                    ...
                    if function_name == "create_event":
                        ...
                        result = self.todo_tools.get_upcoming_tasks()
                    elif function_name == "put_task":
                        result = self.todo_tools.put_task(**function_args)
                ...
                if response.candidates and response.candidates[0].content:
                    current_request_conversation += response.candidates[0].content.parts + function_responses
            ...
            return "Sorry, I encountered an error while processing your request."
                ...
                if uploaded_file.uri:
                    self.client.files.delete_file(uploaded_file.uri)
